...
private class Day07 {
    ...
    fun example2(): Int = bestSpot(parseInput(exampleInput), ::compareFuel)
    fun puzzle2(): Int = bestSpot(parseInput(puzzleInput), ::compareFuel)

    fun parseInput(input: List<String>): List<Int> = input.first().split(",").map { it.toInt() }

    private fun bestSpot(input: List<Int>, compare: (Int, Int) -> Int): Int =
        (input.smallest()..input.largest()).minOf { input.totalDistanceFrom(it, compare) }

    private fun List<Int>.totalDistanceFrom(target: Int, compare: (Int, Int) -> Int): Int =
        foldRight(0) { i, acc ->
            acc + compare(maxOf(target, i), minOf(target, i))
        }

    private val memo = mutableMapOf<Int, Int>()
    fun compareFuel(big: Int, small: Int): Int {
